...
from navsim.agents.goalflow.safety_barrier import SafetyBarrier

import os

class GoalFlowTrajModelBf(nn.Module):
    def __init__(self, config: GoalFlowConfig):
        ...
        self.safety_barrier = SafetyBarrier(
            pixel_size=self._config.bev_pixel_size,
            ego_radius=self._config.safety_ego_radius,
        )

    def solve_safety_correction(self, v_raw, h_val, h_grad, alpha=0.1):
        """
        Solves the CBF-QP analytically (Eq. 8 from SafeFlow paper).
        
        Args:
            v_raw:  [B, N, 2] Predicted flow velocity (in normalized space)
            h_val:  [B, N, 1] Barrier value (h > 0 is safe)
            h_grad: [B, N, 2] Barrier gradient (points toward increasing safety)
            alpha:  Class-K function gain (smaller = less aggressive correction)
        
        Returns:
            u_correction: [B, N, 2] Safety correction to add to v_raw
        
        The constraint is: ∇h · (v + u) + α·h ≥ 0
        Rearranged: ∇h · u ≥ -(∇h · v + α·h)
        
        We find minimal ||u|| that satisfies this via projection.
        """
        # Compute how much the raw velocity violates the constraint
        # L_v = ∇h · v (Lie derivative of h along v)
        L_v = torch.sum(h_grad * v_raw, dim=-1, keepdim=True)
        
        # constraint_violation > 0 means we need correction
        # The constraint is: L_v + alpha * h >= 0
        # violation = -(L_v + alpha * h) = how much we fall short
        constraint_violation = -(L_v + alpha * h_val)
        
        # Only correct if violation > 0 (we're heading toward unsafe region)
        # AND h_val < some threshold (we're actually near the boundary)
        # This prevents corrections when we're safely inside
        near_boundary = (h_val < 2.0).float()  # Only correct within 2m of boundary
        
        grad_norm_sq = torch.sum(h_grad ** 2, dim=-1, keepdim=True) + 1e-8
        
        # Minimal correction: u = λ * ∇h where λ = max(0, violation) / ||∇h||²
        lam = F.relu(constraint_violation) / grad_norm_sq
        
        # Apply near-boundary mask and clamp correction magnitude
        lam = lam * near_boundary
        lam = torch.clamp(lam, max=1.0)  # Limit correction magnitude
        
        u_correction = lam * h_grad
        
        # Clamp total correction to prevent wild jumps
        u_norm = torch.norm(u_correction, dim=-1, keepdim=True)
        max_correction = 0.5  # Maximum correction per step in metric space
        scale = torch.clamp(max_correction / (u_norm + 1e-8), max=1.0)
        u_correction = u_correction * scale
    ...
    def forward(self, features: Dict[str, torch.Tensor], targets: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        # =================================== flow sampling ==================================================
            ...
            sdf_map = self.safety_barrier.compute_sdf(bev_semantic_map)
            ...
            if self._config.fusion:
                ...
                for t_curr, t_prev in zip(t_shifted[:-1], t_shifted[1:]):
                    ...
                    # Get current trajectory positions in metric space for barrier evaluation
                    current_trajs_metric = self.denormalize_xy_rotation(trajs)[..., :2]
                    
                    # Expand SDF map to match trajectory batch dimension
                    # sdf_map: [B, 1, H, W] -> [B * anchor_size, 1, H, W]
                    sdf_map_expanded = sdf_map.unsqueeze(1).expand(-1, self._config.anchor_size, -1, -1, -1)
                    sdf_map_expanded = sdf_map_expanded.reshape(batch_size * self._config.anchor_size, 1, sdf_map.shape[-2], sdf_map.shape[-1])
                    
                    # Get barrier values and gradients in metric space
                    h_val, h_grad = self.safety_barrier.get_gradients(sdf_map_expanded, current_trajs_metric)

                    # The flow v_xy is in normalized space, we need to convert to metric for CBF
                    # Normalized -> Metric: x_metric = x_norm * 60, y_metric = y_norm * 15
                    v_xy_norm = net_output_cond[..., :2]
                    v_xy_metric = v_xy_norm.clone()
                    v_xy_metric[..., 0] = v_xy_norm[..., 0] * 60.0  # x (forward)
                    v_xy_metric[..., 1] = v_xy_norm[..., 1] * 15.0  # y (lateral)

                    # Compute safety correction in metric space
                    u_xy_metric = self.solve_safety_correction(v_xy_metric, h_val, h_grad)

                    # Convert correction back to normalized space
                    u_xy_norm = u_xy_metric.clone()
                    u_xy_norm[..., 0] = u_xy_metric[..., 0] / 60.0
                    u_xy_norm[..., 1] = u_xy_metric[..., 1] / 15.0

                    # Apply correction in normalized space
                    u_full = torch.zeros_like(net_output_cond)
                    u_full[..., :2] = u_xy_norm

                    # Update Flow: v_final = v_raw + u
                    total_flow = net_output_cond + u_full

                    trajs=trajs.detach().clone()+total_flow*(step / self._config.infer_steps)
